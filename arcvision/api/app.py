...
from arcvision.api.auth import (
    allow_any_authenticated,
    allow_public,
    get_allowed_cameras_for_filter,
    require_role,
)
from arcvision.api.config_util import (
    publish_camera_section_updates,
    swap_runtime_config,
)
from arcvision.api.defs.query.app_query_parameters import AppTimelineHourlyQueryParameters
from arcvision.api.defs.request.app_body import (
    AppConfigSetBody,
    GenAIProbeBody,
    MediaSyncBody,
)
from arcvision.api.defs.tags import Tags
from arcvision.config import ArcVisionConfig, GenAIConfig, GenAIProviderEnum
...
from arcvision.const import (
    ...
    REDACTED_CREDENTIAL_SENTINELS,
)
from arcvision.ffmpeg_presets import FFMPEG_HWACCEL_VAAPI, _gpu_selector
from arcvision.genai import PROVIDERS, load_providers
from arcvision.jobs.media_sync import (
    get_current_media_sync_job,
    get_media_sync_job_by_id,
    start_media_sync_job,
)
from arcvision.models import Event, Timeline
from arcvision.stats.prometheus import get_metrics, update_metrics
from arcvision.types import JobStatusTypesEnum
from arcvision.util.builtin import (
...
from arcvision.util.config import (
...
from arcvision.util.time import get_tz_modifiers
...
def config_schema(request: Request):
    return JSONResponse(content=get_config_schema(ArcVisionConfig))
...
async def genai_probe(request: Request, body: GenAIProbeBody):
    ...
    if api_key in REDACTED_CREDENTIAL_SENTINELS:
        saved_cfg = (
...
def config(request: Request):
    config_obj: ArcVisionConfig = request.app.frigate_config
...
def config_save(save_option: str, body: Any = Body(media_type="text/plain")):
    ...
    try:
        ...
        ArcVisionConfig.parse_yaml(new_config)
...
def _restore_masked_camera_paths(config_data: dict, config: ArcVisionConfig) -> None:
...
def _config_set_in_memory(request: Request, body: AppConfigSetBody) -> JSONResponse:
    ...
    try:
        ...
        config: ArcVisionConfig = request.app.frigate_config
    ...
    try:
        with lock:
            ...
            try:
                ...
                if query_string:
                    ...
                    updates = {
                        k: v
                        for k, v in updates.items()
                        if v not in REDACTED_CREDENTIAL_SENTINELS
                    }
                ...
                try:
                    config = ArcVisionConfig.parse(new_raw_config)
                except ValidationError as e:
                    with open(config_file, "w") as f:
            ...
            if body.requires_restart == 0 or body.update_topic:
                old_config: ArcVisionConfig = request.app.frigate_config
                ...
                if body.update_topic:
                    if body.update_topic.startswith("config/cameras/"):
                        ...
                        # global topic the global object. ArcVisionConfig.parse
