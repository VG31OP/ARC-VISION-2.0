...
from arcvision.const import EXPORT_DIR
from arcvision.models import Export, User
...
def resolve_media_uri(
    uri: str, frigate_config: ArcVisionConfig | None = None
...
def _resolve_clip(
    parts: list[str], frigate_config: ArcVisionConfig | None
) -> tuple[MediaAuthResolution, str | None]:
...
def _longest_prefix_camera(
    stem: str, frigate_config: ArcVisionConfig | None
) -> str | None:
...
def _camera_from_clip_filename(
    filename: str, frigate_config: ArcVisionConfig | None
) -> str | None:
...
def check_camera_access(role: str, camera: str, frigate_config: ArcVisionConfig) -> bool:
...
def is_role_restricted(role: str, frigate_config: ArcVisionConfig) -> bool:
...
def deny_response_for_media_uri(
    original_url: str | None, role: str | None, frigate_config: ArcVisionConfig
) -> int | None:
