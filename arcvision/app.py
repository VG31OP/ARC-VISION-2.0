...
from arcvision.api.auth import hash_password
from arcvision.api.fastapi_app import create_fastapi_app
from arcvision.camera import CameraMetrics, PTZMetrics
from arcvision.camera.maintainer import CameraMaintainer
from arcvision.comms.base_communicator import Communicator
from arcvision.comms.dispatcher import Dispatcher
from arcvision.comms.event_metadata_updater import EventMetadataPublisher
from arcvision.comms.inter_process import InterProcessCommunicator
from arcvision.comms.mqtt import MqttClient
from arcvision.comms.object_detector_signaler import DetectorProxy
from arcvision.comms.webpush import WebPushClient
from arcvision.comms.ws import WebSocketClient
from arcvision.comms.zmq_proxy import ZmqProxy
from arcvision.config.camera.updater import CameraConfigUpdatePublisher
from arcvision.config.config import ArcVisionConfig, ArcVisionConfig
from arcvision.config.holder import ConfigHolder
from arcvision.config.profile_manager import ProfileManager
...
from arcvision.data_processing.types import DataProcessorMetrics
from arcvision.db.sqlitevecq import SqliteVecQueueDatabase
from arcvision.debug_replay import (
    DebugReplayManager,
    cleanup_replay_cameras,
)
from arcvision.embeddings import EmbeddingProcess, EmbeddingsContext
from arcvision.events.audio import AudioProcessor
from arcvision.events.cleanup import EventCleanup
from arcvision.events.maintainer import EventProcessor
from arcvision.jobs.export import reap_stale_exports
from arcvision.jobs.motion_search import stop_all_motion_search_jobs
from arcvision.log import _stop_logging
from arcvision.models import (
    Event,
    Export,
    Previews,
    Recordings,
...
from arcvision.object_detection.base import ObjectDetectProcess
from arcvision.output.output import OutputProcess
from arcvision.ptz.autotrack import PtzAutoTrackerThread
from arcvision.ptz.onvif import OnvifController
from arcvision.record.cleanup import RecordingCleanup
from arcvision.record.export import migrate_exports
from arcvision.record.record import RecordProcess
from arcvision.review.review import ReviewProcess
from arcvision.stats.emitter import StatsEmitter
from arcvision.stats.util import stats_init
from arcvision.storage import StorageMaintainer
from arcvision.timeline import TimelineProcessor
from arcvision.track.object_processing import TrackedObjectProcessor
from arcvision.util.builtin import empty_and_close_queue
from arcvision.util.image import UntrackedSharedMemory
from arcvision.util.process import ArcVisionProcess
from arcvision.util.services import set_file_limit
from arcvision.version import VERSION
from arcvision.watchdog import ArcVisionWatchdog
...
class ArcVisionApp:
    def __init__(
        self, config: ArcVisionConfig, manager: SyncManager, stop_event: MpEvent
    ) -> None:
    ...
    @property
    def config(self) -> ArcVisionConfig:
    ...
    def init_database(self) -> None:
        ...
        # Register legacy module aliases so historical migrations referencing frigate.models resolve cleanly
        import arcvision.models
        sys.modules.setdefault("frigate", sys.modules["arcvision"])
        sys.modules.setdefault("frigate.models", sys.modules["arcvision.models"])
        ...
        if len(router.diff) > 0:
            ...
            shutil.copyfile(
                ...
                self.config.database.path.replace("arcvision.db", "backup.db"),
            )
    ...
    def start_watchdog(self) -> None:
        self.watchdog = ArcVisionWatchdog(self.detectors, self.stop_event)

        # (attribute on self, key in self.processes, factory)
        specs: list[tuple[str, str, Callable[[], ArcVisionProcess]]] = [
            (
                "embedding_process",
                "embeddings",
        ...
        for attr, key, factory in specs:
            ...
            def on_restart(
                proc: ArcVisionProcess, _attr: str = attr, _key: str = key
            ) -> None:
    ...
    def start(self) -> None:
        logger.info(f"Starting ARC VISION ({VERSION})")
    ...
    def stop(self) -> None:
        ...
        Path("/dev/shm/.arcvision-is-stopping").touch()
        Path("/dev/shm/.frigate-is-stopping").touch()  # backward compat
