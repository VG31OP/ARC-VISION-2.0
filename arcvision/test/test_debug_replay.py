...
class TestDebugReplayManagerSession(unittest.TestCase):
    def test_inactive_by_default(self) -> None:
        from arcvision.debug_replay import DebugReplayManager
...
class TestDebugReplayManagerStop(unittest.TestCase):
    def setUp(self) -> None:
        ...
        patcher = patch("arcvision.debug_replay.JobStatePublisher")
        patcher.start()
    ...
    def test_stop_publishes_remove_when_camera_was_published(self) -> None:
        from arcvision.config.camera.updater import CameraConfigUpdateEnum
        from arcvision.debug_replay import DebugReplayManager
        ...
        with (
            ...
            patch("arcvision.debug_replay.cancel_debug_replay_job", return_value=False),
        ):
            manager.stop(frigate_config=frigate_config, config_publisher=publisher)
    ...
    def test_stop_skips_remove_publish_when_camera_not_in_config(self) -> None:
        ...
        with (
            patch.object(manager, "_cleanup_db"),
            patch.object(manager, "_cleanup_files"),
            patch("arcvision.debug_replay.cancel_debug_replay_job", return_value=True),
        ):
            manager.stop(frigate_config=frigate_config, config_publisher=publisher)
    ...
    def test_stop_calls_cancel_debug_replay_job(self) -> None:
        ...
        with (
            ...
            patch.object(manager, "_cleanup_files"),
            patch(
                "arcvision.debug_replay.cancel_debug_replay_job",
                return_value=True,
            ) as mock_cancel,
        ):
            manager.stop(frigate_config=frigate_config, config_publisher=publisher)
...
class TestDebugReplayManagerPublishCamera(unittest.TestCase):
    def test_publish_camera_invokes_publisher_with_add_topic(self) -> None:
        ...
        with (
            ...
            patch("arcvision.debug_replay.find_config_file", return_value="/cfg.yml"),
            patch("arcvision.debug_replay.YAML") as yaml_cls,
            patch("arcvision.debug_replay.ArcVisionConfig.parse_object") as parse_object,
    ...
    def test_publish_camera_wraps_parse_failure_in_runtime_error(self) -> None:
        ...
        with (
            ...
            patch("arcvision.debug_replay.YAML") as yaml_cls,
            patch(
                "arcvision.debug_replay.ArcVisionConfig.parse_object",
