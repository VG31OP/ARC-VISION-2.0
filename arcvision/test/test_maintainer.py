...
_MOCKED_MODULES = [
    "arcvision.comms.inter_process",
    "arcvision.comms.detections_updater",
    "arcvision.comms.recordings_updater",
    "arcvision.config.camera.updater",
]
...
from arcvision.config import ArcVisionConfig  # noqa: E402
from arcvision.record.maintainer import RecordingMaintainer  # noqa: E402
...
class TestMaintainer(unittest.IsolatedAsyncioTestCase):
    async def test_move_files_survives_bad_filename(self):
        config = MagicMock(spec=ArcVisionConfig)
        config.cameras = {}
        ...
        with patch("os.listdir", return_value=files):
            with patch("os.path.isfile", return_value=True):
                with patch(
                    "arcvision.record.maintainer.psutil.process_iter", return_value=[]
                ):
                    with patch("arcvision.record.maintainer.logger.warning") as warn:
