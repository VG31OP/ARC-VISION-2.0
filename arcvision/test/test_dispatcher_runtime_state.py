...
from arcvision.app import FrigateApp
...
def _build_dispatcher(cameras: dict[str, MagicMock]) -> Dispatcher:
    ...
    with (
        patch("arcvision.comms.dispatcher.CameraActivityManager"),
        patch("arcvision.comms.dispatcher.AudioActivityManager"),
    ):
...
class TestHandlersPersistViaSet(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with open(self.config_path, "w") as f:
            f.write("")
        self._patcher = patch(
            "arcvision.comms.runtime_state.find_config_file",
...
class TestStartupAppliesConfigLayersBeforeWorkersStart(unittest.TestCase):
    ...
    def _start_call_order(self) -> list[str]:
        ...
        with (
            patch("arcvision.app.set_file_limit"),
            patch("arcvision.app.cleanup_replay_cameras"),
            patch("arcvision.app.reap_stale_exports"),
            patch("arcvision.app.create_fastapi_app"),
            patch("arcvision.app.uvicorn"),
        ):
