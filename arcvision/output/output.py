...
from arcvision.comms.ws import WebSocket
...
from arcvision.output.birdseye import Birdseye
from arcvision.output.camera import JsmpegCamera
from arcvision.output.preview import PreviewRecorder
from arcvision.output.ws_auth import ws_has_camera_access
from arcvision.util.image import SharedMemoryFrameManager, get_blank_yuv_frame
...
class OutputProcess(ArcVisionProcess):
    def __init__(self, config: ArcVisionConfig, stop_event: MpEvent) -> None:
        super().__init__(
            stop_event, PROCESS_PRIORITY_MED, name="arcvision.output", daemon=True
        )
        self.config = config
