...
from arcvision.config.camera.record import ChaptersEnum
from arcvision.const import UPDATE_JOB_STATE
from arcvision.jobs.job import Job
from arcvision.models import Export
from arcvision.record.export import PlaybackSourceEnum, RecordingExporter
...
def _get_max_concurrent(config: ArcVisionConfig) -> int:
...
def get_export_job_manager(config: ArcVisionConfig) -> ExportJobManager:
...
def start_export_job(config: ArcVisionConfig, job: ExportJob) -> str:
    """Queue an export job and return its ID."""
    return get_export_job_manager(config).enqueue(job)


def get_export_job(config: ArcVisionConfig, job_id: str) -> ExportJob | None:
    """Get a queued or completed export job by ID."""
    return get_export_job_manager(config).get_job(job_id)


def list_active_export_jobs(config: ArcVisionConfig) -> list[ExportJob]:
...
def cancel_queued_export_jobs_for_case(
    config: ArcVisionConfig, case_id: str
) -> list[ExportJob]:
...
def available_export_queue_slots(config: ArcVisionConfig) -> int:
