...
from arcvision.app import ArcVisionApp, FrigateApp
from arcvision.config import ArcVisionConfig, ArcVisionConfig
from arcvision.log import setup_logging
...
def main() -> None:
    ...
    threading.current_thread().name = "arcvision"
    stop_event = mp.Event()
    ...
    parser = argparse.ArgumentParser(
        prog="arcvision",
        description="ARC VISION Core Engine - AI Surveillance & NVR Platform.",
    )
    ...
    try:
        config = ArcVisionConfig.load(install=True)
        ...
        try:
            config = ArcVisionConfig.load(install=True, safe_load=True)
            print("Starting ARC VISION in safe mode.")
        except ValidationError:
            print("Unable to start ARC VISION in safe mode.")
            sys.exit(1)
    ...
    ArcVisionApp(config, manager, stop_event).start()
