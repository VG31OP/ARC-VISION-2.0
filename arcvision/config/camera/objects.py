...
class FilterConfig(ArcVisionBaseModel):
...
class GenAIObjectTriggerConfig(ArcVisionBaseModel):
...
class GenAIObjectConfig(ArcVisionBaseModel):
    enabled: bool = Field(
        default=False,
        title="Enable GenAI",
...
class ObjectConfig(ArcVisionBaseModel):
