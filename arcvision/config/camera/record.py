...
from arcvision.const import MAX_PRE_CAPTURE
...
class RecordRetainConfig(ArcVisionBaseModel):
    days: float = Field(
        default=0,
        ge=0,
...
class ReviewRetainConfig(ArcVisionBaseModel):
    days: float = Field(
        default=10,
        ge=0,
        title="Retention days",
...
class EventsConfig(ArcVisionBaseModel):
...
class RecordPreviewConfig(ArcVisionBaseModel):
...
class RecordExportConfig(ArcVisionBaseModel):
...
class RecordConfig(ArcVisionBaseModel):
    enabled: bool = Field(
        default=False,
