...
from arcvision.const import REGEX_JSON
from arcvision.detectors import DetectorConfig, ModelConfig
from arcvision.detectors.detector_config import BaseDetectorConfig
...
from arcvision.util.image import create_mask
from arcvision.util.services import auto_detect_hwaccel
...
__all__ = ["ArcVisionConfig", "ArcVisionConfig"]
...
def verify_valid_live_stream_names(
    frigate_config: ArcVisionConfig, camera_config: CameraConfig
) -> ValueError | None:
...
class ArcVisionConfig(ArcVisionBaseModel):
    ...
    safe_mode: bool = Field(
        default=False,
        title="Safe mode",
        description="When enabled, start ARC VISION in safe mode with reduced features for troubleshooting.",
    )
    ...
    def load(cls, **kwargs):
        ...
        with open(config_path, "a+" if new_config else "r") as f:
            ...
            f.seek(0)
            return ArcVisionConfig.parse(f, **kwargs)
...
FrigateConfig = ArcVisionConfig
FrigateBaseModel = ArcVisionBaseModel
