...
def get_config_schema(config_class: type[BaseModel]) -> dict[str, Any]:
    """
    Returns the JSON schema for ArcVisionConfig with polymorphic detectors.

    This utility patches the ArcVisionConfig schema to include the full polymorphic
    ...
    from arcvision.detectors import DetectorConfig

    # Get the base schema for ArcVisionConfig
    ...
    # Ensure $defs exists in ArcVisionConfig schema
    if "$defs" not in schema:
        schema["$defs"] = {}

    # Merge $defs from DetectorConfig into ArcVisionConfig schema
