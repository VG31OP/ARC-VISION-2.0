...
import arcvision.log
from arcvision.config.logger import LoggerConfig
...
class ArcVisionProcess(BaseProcess):
    logger: logging.Logger

    def before_start(self) -> None:
        self.__log_queue = arcvision.log.log_listener.queue
    ...
    def pre_run_setup(self, logConfig: LoggerConfig | None = None) -> None:
        ...
        if logConfig:
            arcvision.log.apply_log_levels(
                logConfig.default.value.upper(), logConfig.logs
            )
        else:
            # Apply default INFO level with standard library suppressions
            arcvision.log.apply_log_levels("INFO", {})

        self._setup_memray()

    def _setup_memray(self) -> None:
        """Setup memray profiling if enabled via environment variable."""
        memray_modules = os.environ.get(
            "ARC_VISION_MEMRAY_MODULES", os.environ.get("FRIGATE_MEMRAY_MODULES", "")
        )

        if not memray_modules:
            return

        # Extract module name from process name (e.g., "arcvision.capture:camera" -> "arcvision.capture")
...
FrigateProcess = ArcVisionProcess
