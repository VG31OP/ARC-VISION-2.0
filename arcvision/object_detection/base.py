...
from arcvision.comms.object_detector_signaler import (
...
from arcvision.detectors import create_detector
from arcvision.detectors.detector_config import (
    BaseDetectorConfig,
    InputDTypeEnum,
    ModelConfig,
)
from arcvision.util.builtin import EventsPerSecond, load_labels
...
class DetectorRunner(ArcVisionProcess):
    def __init__(
        self,
        name: str,
...
class AsyncDetectorRunner(ArcVisionProcess):
    def __init__(
        self,
        name: str,
        detection_queue: Queue,
...
class ObjectDetectProcess:
    def __init__(
        ...
        self.detect_process: ArcVisionProcess | None = None
        self.config = config
        self.detector_config = detector_config
    ...
    def start_or_restart(self) -> None:
        ...
        if self.detector_config.type == "memryx":
            self.detect_process = AsyncDetectorRunner(
                f"arcvision.detector:{self.name}",
