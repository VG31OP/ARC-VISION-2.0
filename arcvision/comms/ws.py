...
def _collect_zone_names(config: ArcVisionConfig) -> set[str]:
...
def _ws_valid_roles(ws: Any, config: ArcVisionConfig) -> list[str]:
...
def _ws_is_unrestricted(ws: Any, config: ArcVisionConfig) -> bool:
...
def _ws_allowed_cameras(ws: Any, config: ArcVisionConfig) -> set[str]:
...
class WebSocketClient(Communicator):
    ...
    def __init__(self, config: ArcVisionConfig) -> None:
        self.config = config
