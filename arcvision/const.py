...
INSTALL_DIR = os.environ.get("ARC_VISION_INSTALL_DIR") or (
    "/opt/arcvision"
    if os.path.exists("/opt/arcvision") or not os.path.exists("/opt/frigate")
    else "/opt/frigate"
)
CONFIG_DIR = os.environ.get("ARC_VISION_CONFIG_DIR") or "/config"

# Database resolution: default to arcvision.db; if frigate.db exists and arcvision.db does not, fall back to frigate.db
DEFAULT_DB_PATH = (
    os.environ.get("ARC_VISION_DATABASE_PATH")
    or os.environ.get("FRIGATE_DATABASE_PATH")
    or (
        f"{CONFIG_DIR}/frigate.db"
        if os.path.exists(f"{CONFIG_DIR}/frigate.db")
        and not os.path.exists(f"{CONFIG_DIR}/arcvision.db")
        else f"{CONFIG_DIR}/arcvision.db"
    )
)

MODEL_CACHE_DIR = f"{CONFIG_DIR}/model_cache"

# Storage base directory: default to /media/arcvision; if /media/frigate exists and /media/arcvision does not, fall back to /media/frigate
BASE_DIR = (
    os.environ.get("ARC_VISION_BASE_DIR")
    or os.environ.get("FRIGATE_BASE_DIR")
    or (
        "/media/frigate"
        if os.path.exists("/media/frigate")
        and not os.path.exists("/media/arcvision")
        else "/media/arcvision"
    )
)
...
REDACTED_CREDENTIAL_SENTINEL = "__ARCVISION_SAVED_CREDENTIAL__"
LEGACY_REDACTED_CREDENTIAL_SENTINEL = "__FRIGATE_SAVED_CREDENTIAL__"
REDACTED_CREDENTIAL_SENTINELS = {
    REDACTED_CREDENTIAL_SENTINEL,
    LEGACY_REDACTED_CREDENTIAL_SENTINEL,
}
